...
'''
    ...
    # Write library.js (external JS for CSP compliance)
    library_js = '''// Configuration
const CONFIG = {
...
function clientSideSearch(query, entries) {
    ...
    return scored.filter(s => s.score > 0).sort((a, b) => b.score - a.score).slice(0, 20).map(s => s.entry);
}
...
function getContentUrl(entry) {
    ...
    if (type === 'legal') return `legal/${filename}.html`;
    if (type === 'law-journal') return `journals/${filename}.html`;
...
function renderSearchResults(results, total, query, isClientSide = false) {
    ...
    searchResults.innerHTML = `<p class="search-count">Found ${total} result${total !== 1 ? 's' : ''} for "${query}"</p>` +
        results.map(r => `<article class="search-result-card"><h2><a href="${getContentUrl(r)}">${r.title}</a></h2>
            <div class="entry-meta"><span class="duration">${formatDuration(r.duration_seconds)}</span>
            ${r.facets?.topics ? ' • <span class="topic">' + r.facets.topics.join(', ') + '</span>' : ''}
            ${r.facets?.difficulty ? ' • <span class="difficulty">' + r.facets.difficulty + '</span>' : ''}</div>
            ${r.summary?.length ? '<div class="entry-summary"><ul>' + r.summary.slice(0, 2).map(s => '<li>' + s + '</li>').join('') + '</ul></div>' : ''}</article>`).join('');
}
...
async function sendChatMessage() {
    ...
    try {
        const response = await fetch(`${CONFIG.API_URL}/api/chat`, {
            ...
            body: JSON.stringify({ model: 'gpt-oss:120b-cloud', messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }], stream: false })
        });
        loadingDiv.remove();
        const result = await response.json();
        const aiResponse = result.message?.content || result.response || 'Sorry, I could not generate a response.';
        let html = aiResponse.replace(/\\*\\*([^*]+)\\*\\*/g, '<strong>$1</strong>').replace(/\\n/g, '<br>');
        if (results.length > 0) {
            html += '<div class="chat-videos"><strong>Related:</strong><ul>' + results.slice(0, 5).map(v => `<li><a href="${getContentUrl(v)}">${v.title}</a></li>`).join('') + '</ul></div>';
        }
        const msgDiv = document.createElement('div');
        ...
        loadingDiv.remove();
        if (results.length > 0) {
            let html = `<p>I found ${results.length} relevant items:</p><ul>` + results.slice(0, 5).map(v => `<li><a href="${getContentUrl(v)}">${v.title}</a></li>`).join('') + '</ul>';
            const msgDiv = document.createElement('div');
            msgDiv.className = 'chat-message assistant';
            msgDiv.innerHTML = html;
            chatMessages.appendChild(msgDiv);
...
document.querySelectorAll('.domain-filter a[data-domain]').forEach(link => {
    link.addEventListener('click', (e) => {
        ...
        document.querySelectorAll('.entry-card').forEach(card => {
            ...
            card.style.display = (activeDomain === 'all' || cardDomain === activeDomain) ? '' : 'none';
        });
    });
});
...
document.querySelectorAll('.tag[data-filter]').forEach(tag => {
    tag.addEventListener('click', (e) => {
        ...
        document.querySelectorAll('.entry-card').forEach(card => {
            let show = true;
            const cardDomain = card.dataset.domain || 'computer-science';
            if (activeDomain !== 'all' && cardDomain !== activeDomain) show = false;
            if (show) {
                for (const [filterName, values] of Object.entries(activeFilters)) {
                    const cardValue = card.dataset[filterName];
                    if (!values.some(v => cardValue.includes(v))) { show = false; break; }
                }
            }
...
'''
    (SITE_DIR / "assets" / "library.js").write_text(library_js)
